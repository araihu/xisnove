...
Xisnove v1 consumes secrets through references or owner-readable files. It does
...
| `xisnove-agent` | Agent credential bundle | `XISNOVE_AGENT_CREDENTIAL_FILE` | owner-only file; operator materialization uses atomic projection and generation metadata |

File readers reject symlinks where the existing secret helper can enforce it,
reject group/world-writable material, bound file size, and redact paths from
public diagnostics. Kubernetes resources refer to existing Secret names and
keys. Compose and raw resources refer to host paths. They never accept a secret
literal in committed configuration.
...
5. Materialize the returned credential once with owner-only permissions.
   Retries reuse the recorded generation and never request an implicit rotation.
