...
func TestBrandUsesX9DisplayName(t *testing.T) {
	...
	if body := rendered.String(); !strings.Contains(body, `aria-label="X-9"`) || !strings.Contains(body, `class="v11-signal"`) || strings.Contains(body, "Xisnove") {
...
func TestDocumentLoadsAraiHuThemeAfterGoshtosoAndUsesItByDefault(t *testing.T) {
	...
	if !strings.Contains(body, `<link rel="icon" type="image/svg+xml" href="/ui/x9-v11-icon-9aef3646.svg">`) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(body, want) {
			t.Errorf("brand theme contract missing %q", want)
		}
	}
