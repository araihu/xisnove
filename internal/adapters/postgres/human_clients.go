...
func (r *idempotencyRepository) Get(
	...
	record, err := r.queries.GetActiveIdempotencyRecord(ctx, dbpostgres.GetActiveIdempotencyRecordParams{
...
func (r *idempotencyRepository) Create(ctx context.Context, record application.IdempotencyRecord) error {
	affected, err := r.queries.PutIdempotencyRecord(ctx, dbpostgres.PutIdempotencyRecordParams{
...
func (r *idempotencyRepository) DeleteExpired(ctx context.Context, now time.Time, limit int) (int64, error) {
	...
	deleted, err := r.queries.DeleteExpiredIdempotencyRecords(ctx, dbpostgres.DeleteExpiredIdempotencyRecordsParams{
		Cutoff: formatTime(now), RowLimit: int32(limit),
	})
	if err != nil {
