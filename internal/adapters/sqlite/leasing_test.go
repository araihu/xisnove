...
func TestCompetingAgentsHaveSingleLeaseWinnerAndExpiredLeaseIsReclaimed(t *testing.T) {
	...
	works := make([]*application.HTTPWork, 2)
	errs := make([]error, 2)
	...
	for i, agentID := range agentIDs {
		...
		go func() {
			defer group.Done()
			<-start
			works[i], errs[i] = service.LeaseHTTP(context.Background(), agentID, 0)
		}()
	}
	close(start)
	group.Wait()
	...
	loser := 1 - winner
	reclaimed, err := service.LeaseHTTP(context.Background(), agentIDs[loser], 0)
	if err != nil {
		t.Fatal(err)
	}
