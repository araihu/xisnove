...
INSERT INTO location_health (
  ...
  sqlc.arg(monitor_id), sqlc.arg(location_id), sqlc.arg(state),
  sqlc.arg(consecutive_failures), sqlc.arg(consecutive_successes),
  sqlc.narg(last_observed_at), sqlc.narg(last_transition_at), sqlc.narg(stale_at)
)
...
WHERE ml.monitor_id = sqlc.arg(monitor_id)
  AND ml.required
...
VALUES (sqlc.arg(monitor_id), sqlc.arg(state), sqlc.narg(last_transition_at))
