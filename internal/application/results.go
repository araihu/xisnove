...
func NewHealthService(store Store) *HealthService {
...
func (s *HealthService) GetMonitorHealth(
	...
	monitor, err := s.store.Repositories().Health.GetMonitor(ctx, monitorID)
	if err != nil {
		return MonitorHealthView{}, err
	}
	locations, err := s.store.Repositories().Health.ListRequiredLocations(ctx, monitorID)
	if err != nil {
		return MonitorHealthView{}, err
	}
...
func (s *HealthService) GetActiveIncident(
	...
	return s.store.Repositories().Incidents.GetActive(ctx, monitorID)
}
